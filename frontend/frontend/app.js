document.addEventListener('DOMContentLoaded', () => {
    ...
    async function processBackendResponse(data) {
        ...
        const text = (data.corrected_transcript || '').toLowerCase();

        // When we are mid-troubleshoot, detect yes/no from the transcript
        // and handle via dedicated endpoints instead of the main pipeline response.
        if (inTroubleshoot) {
        ...
        // Still mid-troubleshoot but response was ambiguous — ask to clarify.
        if (inTroubleshoot) {
        ...
        // Fresh issue — use the main pipeline result.
        ...
        if (currentState === 'ready') {
            try {
                ...
                mediaRecorder.onstop = async () => {
                    ...
                    console.log(`[Audio] ${audioBlob.size} bytes, ${supportedMime}`);
